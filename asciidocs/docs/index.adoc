...
Da wir in mit mehreren Threads arbeiten, die zugleich zugriff, auf die Variable message zugreifen müssen wir diese Variable auf einem Globalen Scope definieren
...
<1> Als Erstes wird unsere statische message Variable befüllt.
<2> Mit dieser Variable wird dann durch den emitter unsere Nachricht an den gRPC Service geschickt.
<3> Die input Zeile wird nun gelöscht und anstatt von `Write Message: ` wird die Zeile mit `You wrote: ` befüllt.
<4> Zuletzt wird unsere Variable message wieder zurückgesetzt.
...
Das bestimmte flags werden vom Terminal entfernt, sodass der Input der in die Konsole geschrieben wird:
...
Dann wird der Input des Users gelesen und in die Variable message gespeichert.
